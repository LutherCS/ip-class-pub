...
def test_json():
    def _test_json(category: str, number: int = 3):
        with open(pathlib.Path(__file__).parent / pathlib.Path("testQuestions.json"), "r") as f:
            data = json.loads(f.read())
        if category:
            data["results"] = [
                question for question in data["results"] if question["category"] == category
            ]
...
def test_no_input(page: Page):
    ...
    expect(page.locator("#numberInputField > p.help")).to_have_text("This field is required")
...
@pytest.mark.parametrize("category", ["Science: Computers", "Mythology", "Geography", "History"])
...
def test_numbers(page: Page, test_json, category: str, number: int):
    ...
    expect(page.locator("#questionsDiv > .card > footer > p:nth-child(1)")).to_have_text(
        [category] * number
    )
...
def test_categories(page: Page, test_json, category: str):
    ...
    page.route("*/**/bulma.min.css", lambda route: route.abort())
    page.route(
        "https://opentdb.com/api.php*", lambda route: route.fulfill(json=test_json(category))
    )
    page.goto("http://localhost:8000/")
    page.select_option("#categorySelect", category)
...
def test_difficulty(page: Page, test_json, category: str):
    ...
    expect(page.locator("#questionsDiv > .card > footer > p:nth-child(2)")).to_have_text(
        ["Easy", "Medium", "Hard"]
    )
...
def test_answers(page: Page, test_json, category: str):
    ...
    expect(page.locator("#questionsDiv > .card > footer > p:nth-child(3) > button")).to_have_count(
        3
    )
    for button in page.locator("#questionsDiv > .card > footer > p:nth-child(3) > button").all():
        button.click()
    expect(
        page.locator("#questionsDiv > .card > div.card-content > ol > li.correct_answer")
    ).to_have_count(3)
